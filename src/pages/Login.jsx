...
const Login = () => {
  ...
  return (
    <div className="w-screen h-screen flex justify-center items-center">
      <form onSubmit={handleLogin} className='flex flex-col justify-center items-center gap-10 font-poppins w-full lg:gap-6'>
        ...
        <div className='flex flex-col gap-3 w-[65%] md:items-center'>
          ...
          <input type="text" id="username" className='border border-black p-3 rounded-md md:w-3/5 lg:w-1/2 xl:w-2/5' placeholder='Enter your Username' value={username} onChange={(e) => setUsername(e.target.value)} required />
          ...
          <div className='w-full flex flex-col items-center relative'>
            <div className="relative w-full md:w-3/5 lg:w-1/2 xl:w-2/5">
              <input type={showPassword ? "text" : "password"} id="password"
                className="w-full border border-black p-3 pr-10 rounded-md" placeholder="Enter your Password" value={password} onChange={(e) => setPassword(e.target.value)} required />
              <button onClick={() => setShowPassword(prev => !prev)} className="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-600" type='button'>
        ...
        <div className='w-[65%] flex justify-center'>
          <Button type="submit" className="bg-blue-button font-light text-md px-10 py-7 w-full md:w-3/5 lg:w-1/2 xl:w-2/5">
            Log In
          </Button>
        </div>
      </form>
