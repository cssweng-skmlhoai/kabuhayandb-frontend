      ...
      <div className="flex flex-col xl:flex-row flex-1 relative">
        ...
        <div className="flex-1 relative">
          <div className="pb-5 pt-8 px-7 flex flex-col gap-10 font-poppins xl:bg-white xl:gap-0 xl:px-5 xl:pt-5">
            <div className="hidden xl:flex justify-between w-full items-end p-5">
              ...
              <div className="flex flex-col">
                <p className="font-semibold text-3xl">
                  Search Member (
                  {`${purpose === "dues" ? "Dues" : "Certification"}`})
                </p>
                <p>
                  {purpose === "dues" && (
                    <>
                  ...
                  Select a Member to {purpose === "dues" ? "Manage" : "Issue"}{" "}
                  their{" "}
            ...
            <div
              className={`flex flex-col gap-8 xl:flex xl:border xl:border-black xl:mr-3 xl:mt-3 xl:mb-10 xl:rounded-lg xl:py-10 ${purpose === "certification" ? "xl:flex-row xl:px-25" : "xl:px-60"}`}
            >
              <div
                className={`flex flex-col gap-5 ${purpose === "certification" ? "xl:w-3/5" : ""}`}
              >
                ...
                <div className="flex flex-col items-center gap-3">
                  ...
                  <p className="text-sm italic text-gray-500">
                    Note: Empty the search bar and press 'Search' to show all
                    members
                  </p>
                </div>

                {members.length === 0 ? (
                  <p className="text-center text-gray-500 mt-4 bg-customgray2 py-4">
                    Search for a Member.
                  </p>
                ) : (
                    ...
                    <div
                      ...
                      onClick={
                        purpose === "certification"
                          ...
                          : () =>
                              navigate(
                                `/${purpose}/${member.member_id}/${member.fullname}`
                              )
                      }
                    >
              ...
              {purpose === "certification" && (
                <div className="flex flex-col items-center gap-3 xl:w-2/5">
                    ...
                    <thead>
                      <tr className="text-center">
                        ...
                        <th className="px-4 py-2 rounded-tr-md">
                          Date Requested{" "}
                          <div className="!font-normal">(yyyy-mm-dd)</div>
                        </th>
                      </tr>
                    </thead>
                    <tbody>
                      {certs.length === 0 ? (
                        ...
                        currentCerts.map((cert) => (
                          <tr
                            className="bg-gray-200 rounded-md cursor-pointer hover:bg-gray-300 duration-200 shadow-md text-center"
                          ...
                          >
                            ...
                            <td className="text-gray-500 text-2xl font-light rounded-r-md py-4 pr-2">
                              &rsaquo;
                            </td>
                          </tr>
                        ))
                  ...
                  <div
                    className={`flex justify-between items-center w-full ${certs.length <= certsPerPage ? "hidden" : ""}`}
                  >
                    <p className="text-sm text-gray-600">
                      {certs.length === 0
                        ? "0 results"
                    ...
                    <div className="flex items-center gap-3">
                      <button
                        onClick={() =>
                          setCertCurrentPage((prev) => Math.max(prev - 1, 1))
                        }
                      ...
                      <button
                        onClick={() =>
                          setCertCurrentPage((prev) =>
                            Math.min(prev + 1, certTotalPages)
                          )
                        }
        ...
        <DialogContent className="w-[80%]">
          ...
          <div className="flex flex-col gap-3 my-5">
            <div className="flex flex-col gap-2 font-medium text-lg">
              <p>
                CRN: <span className="font-normal">{crn}</span>
              </p>
              <p>
                Member Name: <span className="font-normal">{memberName}</span>
              </p>
              <p>
                Date Requested (yyyy-mm-dd):{" "}
                <span className="font-normal">{formatDate(datePrinted)}</span>
              </p>
            </div>
          </div>
          <DialogFooter className="flex flex-row">
            <button
              className="bg-red-700 w-1/2 text-white py-3 rounded-md hover:bg-red-900 duration-200"
              onClick={() => setDeleteCertReqDialog(true)}
            >
              Delete Record
            </button>
        ...
        <DialogContent className="w-[80%]">
          <DialogHeader>
            ...
            <DialogDescription className="text-md text-gray-700">
              Are you sure you want to delete this certification request record?
              Double check before performing this action
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
      ...
      <Dialog open={addCertDialog} onOpenChange={setAddCertDialog}>
        <DialogContent className="w-[80%]">
          <DialogHeader>
            ...
            <DialogDescription className="text-md text-gray-700">
              This will create a certification request record and will proceed
              to the selected member's certification
            </DialogDescription>
          </DialogHeader>
