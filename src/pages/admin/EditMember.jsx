...
import {
  ...
  handleUpdates,
  formatDate
} from "@/hooks/EditMemberUtils";

const EditMember = ({ view }) => {
  const { id } = useParams();
  ...
  useEffect(() => {
    fetchMemberInfo(id, setAllDetails, setMemberData, setFamilyData, setHouseholdData, setFamilyMembers);
  }, [id]);
  ...
  return (
    <div>
      ...
      >
        <div className="flex flex-col gap-4 xl:col-start-1">
          ...
          <div className="bg-white p-5 flex flex-col rounded-md font-poppins font-normal">
            <TextInput label="Last Name" value={memberData.last_name} onChange={e => setMemberData({ ...memberData, last_name: e.target.value })} required disabled={!isEdit} placeholder="Last Name" />
            <TextInput label="First Name" value={memberData.first_name} onChange={e => setMemberData({ ...memberData, first_name: e.target.value })} required disabled={!isEdit} placeholder="First Name" />
            <TextInput label="Middle Name" value={memberData.middle_name} onChange={e => setMemberData({ ...memberData, middle_name: e.target.value })} required disabled={!isEdit} placeholder="Middle Name" />
            <TextInput label="Date of Birth" type="date" value={formatDate(memberData.birth_date)} onChange={e => setMemberData({ ...memberData, birth_date: e.target.value })} disabled={!isEdit} />

            <div className="flex justify-between gap-4">
              <div className="flex flex-col w-1/2">
                <TextInput label="Age" value={allDetails.age || ""} readOnly disabled placeholder="00" />
              </div>
              <div className="flex flex-col w-1/2">
                <SelectInput label="Gender" value={memberData.gender} onChange={e => setMemberData({ ...memberData, gender: e.target.value })} options={["Male", "Female", "Other"]} disabled={!isEdit} />
              </div>
            </div>
            <TextInput label="Position" value={familyData.head_position} onChange={e => setFamilyData({ ...familyData, head_position: e.target.value })} required disabled={!isEdit} placeholder="Position" />
            <TextInput label="Contact Number" type="number" value={memberData.contact_number} onChange={e => setMemberData({ ...memberData, contact_number: e.target.value })} required disabled={!isEdit} placeholder="Contact Number" />
          </div>

          <div className="bg-white p-5 flex flex-col rounded-md font-poppins font-normal">
            <TextInput label="TCT No." value={householdData.tct_no} onChange={e => setHouseholdData({ ...householdData, tct_no: e.target.value })} required disabled={!isEdit} placeholder="TCT Number" />
            <div className="flex justify-between gap-4">
              <div className="flex flex-col w-[48%]">
                <TextInput label="Block No." type="number" value={householdData.block_no} onChange={e => setHouseholdData({ ...householdData, block_no: e.target.value })} required disabled={!isEdit} placeholder="Block Number" />
              </div>
              <div className="flex flex-col w-[48%]">
                <TextInput label="Lot No." type="number" value={householdData.lot_no} onChange={e => setHouseholdData({ ...householdData, lot_no: e.target.value })} required disabled={!isEdit} placeholder="Lot Number" />
              </div>
            </div>
            <TextInput label="Share of Open Space" type="number" value={householdData.open_space_share} onChange={e => setHouseholdData({ ...householdData, open_space_share: e.target.value })} required disabled={!isEdit} placeholder="Open Space Share" />
            <TextInput label="Area" type="number" value={householdData.area} onChange={e => setHouseholdData({ ...householdData, area: e.target.value })} required disabled={!isEdit} />
            {"total" in allDetails && <TextInput label="Total" value={allDetails.total} readOnly disabled placeholder="Area" />}
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4 xl:col-start-2">
          ...
          <FamilyAccordion
            ...
            onChange={(key, field, value) =>
              handleFamilyMemberChange(setFamilyMembers, key, field, value)
            }
            onRemove={(key, name) =>
              confirmRemoveFamilyMember(setMemberToDeleteId, setMemberToDeleteName, setDeleteDialogOpen, key, name)
            }
          />
        </div>
        ...
        <div className="flex flex-col gap-4 xl:col-start-3">
          <div className="bg-white p-5 flex flex-col rounded-md font-poppins font-normal">
            <FileUpload
              ...
              disabled={!isEdit}
            />
            <TextInput label="Remarks" value={memberData.remarks || ""} onChange={e => setMemberData({ ...memberData, remarks: e.target.value })} disabled={!isEdit} />
          </div>
            ...
            <div className="flex justify-between mb-3">
              {["Meralco", "Maynilad", "Septic_Tank"].map((key) => (
                <div key={key} className="flex flex-col items-center gap-1">
                  ...
                  <Checkbox
                    id={key}
                    className={`border bg-customgray1 size-6 ${!isEdit ? "pointer-events-none opacity-100" : ""}`}
      ...
      <ConfirmationDialog
        ...
        onConfirm={() => handleDeleteConfirmed(setDeleteDialogOpen, setFamilyMembers, memberToDeleteId)}
      ...
      <ConfirmationDialog
        ...
        setOpen={setConfirmEditDialog}
        title={"Edit Member?"}
        description={"Proceed to edit this member? Please double-check the details before proceeding."}
        confirmText="Proceed"
        onConfirm={(e) => handleUpdates(e, id, memberData, familyData, householdData, familyMembers, navigate)}
      />
    </div>
  );
};
